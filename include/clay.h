...
#define CLAY_SIDI_LOCAL(label, index)                                          \
  Clay__HashStringWithOffset(label, index, Clay_GetOpenElementId())
...
#define CLAY_TEXT(text, ...)                                                   \
  Clay__OpenTextElement(                                                       \
      text, CLAY__CONFIG_WRAPPER(Clay_TextElementConfig, __VA_ARGS__))
...
typedef struct Clay_SizingMinMax {
  float min; // The smallest final size of the element on this axis will be this
             // value in pixels.
  float max; // The largest final size of the element on this axis will be this
             // value in pixels.
} Clay_SizingMinMax;
  ...
  union {
    Clay_SizingMinMax
        minMax; // Controls the minimum and maximum size in pixels that this
                // element is allowed to grow or shrink to, overriding sizing
                // types such as FIT or GROW.
    float
        percent; // Expects 0-1 range. Clamps the axis size to a percent of the
                 // parent container's axis size minus padding and child gaps.
  } size;
...
typedef struct Clay_LayoutConfig {
  ...
  Clay_Padding padding; // Controls "padding" in pixels, which is a gap between
                        // the bounding box of this element and where its
                        // children will be placed.
  uint16_t childGap; // Controls the gap in pixels between child elements along
                     // the layout axis (horizontal gap for LEFT_TO_RIGHT,
                     // vertical gap for TOP_TO_BOTTOM).
  Clay_ChildAlignment
      childAlignment; // Controls how child elements are aligned on each axis.
  Clay_LayoutDirection
      layoutDirection; // Controls the direction in which child elements will be
...
typedef struct Clay_FloatingAttachPoints {
  Clay_FloatingAttachPointType
      element; // Controls the origin point on a floating element that attaches
               // to its parent.
  Clay_FloatingAttachPointType
...
typedef struct Clay_ClipElementConfig {
  ...
  Clay_Vector2 childOffset; // Offsets the x,y positions of all child elements.
...
typedef struct Clay_BorderElementConfig {
  Clay_Color
      color; // Controls the color of all borders with width > 0. Conventionally
             // represented as 0-255, but interpretation is up to the renderer.
  Clay_BorderWidth
      width; // Controls the widths of individual borders. At least one of these
...
typedef enum {
  ...
  CLAY_TRANSITION_PROPERTY_BORDER = CLAY_TRANSITION_PROPERTY_BORDER_COLOR |
...
typedef struct Clay_TransitionElementConfig {
  ...
  struct {
    Clay_TransitionData (*setFinalState)(Clay_TransitionData initialState,
                                         Clay_TransitionProperty properties);
...
// Render command data when commandType ==
// CLAY_RENDER_COMMAND_TYPE_OVERLAY_COLOR_START || commandType ==
...
typedef union Clay_RenderData {
  ...
  // Render command data when commandType ==
  // CLAY_RENDER_COMMAND_TYPE_OVERLAY_COLOR_START|END
...
typedef struct Clay_ScrollContainerData {
  ...
  // Indicates whether an actual scroll container matched the provided ID or if
  // the default struct was returned.
  bool found;
} Clay_ScrollContainerData;

// Bounding box and other data for a specific UI element.
typedef struct Clay_ElementData {
  // The rectangle that encloses this UI element, with the position relative to
...
typedef CLAY_PACKED_ENUM{
    ...
    CLAY_RENDER_COMMAND_TYPE_SCISSOR_END,
    // The renderer should begin performing a "color overlay" on all subsequent
    // render commands until disabled again.
    CLAY_RENDER_COMMAND_TYPE_OVERLAY_COLOR_START,
    // The renderer should disable any previously active "color overlay" and
    // render elements with their standard colors again.
    CLAY_RENDER_COMMAND_TYPE_OVERLAY_COLOR_END,
...
typedef struct Clay_RenderCommand {
  ...
  // CLAY_RENDER_COMMAND_TYPE_RECTANGLE - The renderer should draw a solid color
  // rectangle. CLAY_RENDER_COMMAND_TYPE_BORDER - The renderer should draw a
  // colored border inset into the bounding box. CLAY_RENDER_COMMAND_TYPE_TEXT -
  // The renderer should draw text. CLAY_RENDER_COMMAND_TYPE_IMAGE - The
  // renderer should draw an image. CLAY_RENDER_COMMAND_TYPE_SCISSOR_START - The
  // renderer should begin clipping all future draw commands, only rendering
  // content that falls within the provided boundingBox.
  // CLAY_RENDER_COMMAND_TYPE_SCISSOR_END - The renderer should finish any
  // previously active clipping, and begin rendering elements in full again.
  // CLAY_RENDER_COMMAND_TYPE_CUSTOM - The renderer should provide a custom
  // implementation for handling this render command based on its .customData
...
typedef struct Clay_ElementDeclaration {
  ...
  Clay_Color backgroundColor;
  // Perform an image editing style "Color Overlay" on this element and all its
  // children, equivalent to glsl mix(elementColor, overlayColor.rgb,
  // overlayColor.a)
  Clay_Color overlayColor;
  // Controls the "radius", or corner rounding of elements, including
...
typedef CLAY_PACKED_ENUM{
    ...
    // were still remaining open elements when the layout ended.
...
typedef struct Clay_ErrorData {
  // Represents the type of error clay encountered while computing layout.
  // CLAY_ERROR_TYPE_TEXT_MEASUREMENT_FUNCTION_NOT_PROVIDED - A text measurement
  // function wasn't provided using Clay_SetMeasureTextFunction(), or the
  // provided function was null. CLAY_ERROR_TYPE_ARENA_CAPACITY_EXCEEDED - Clay
  // attempted to allocate its internal data structures but ran out of space.
  // The arena passed to Clay_Initialize was created with a capacity smaller
  // than that required by Clay_MinMemorySize().
  // CLAY_ERROR_TYPE_ELEMENTS_CAPACITY_EXCEEDED - Clay ran out of capacity in
  // its internal array for storing elements. This limit can be increased with
  ...
  // CLAY_ERROR_TYPE_DUPLICATE_ID - Two elements were declared with exactly the
  // same ID within one layout.
  ...
  // CLAY_ERROR_TYPE_PERCENTAGE_OVER_1 - An element was declared that using
  // CLAY_SIZING_PERCENT but the percentage value was over 1. Percentage values
  // are expected to be in the 0-1 range. CLAY_ERROR_TYPE_INTERNAL_ERROR - Clay
  // encountered an internal error. It would be wonderful if you could report
  ...
  // A transparent pointer passed through from when the error handler was first
  // provided.
  void *userData;
} Clay_ErrorData;
...
CLAY_DLL_EXPORT void Clay_SetPointerState(Clay_Vector2 position,
                                          bool pointerDown);
// Returns the state of the "pointer" (i.e. the mouse or touch) which was set
// via Clay_SetPointerState().
...
// Gets the ID of the currently open element, useful for retrieving IDs
// generated by CLAY_AUTO_ID()
...
#define CLAY__ARRAY_DEFINE_FUNCTIONS(typeName, arrayName)                      \
  ...
  arrayName arrayName##_Allocate_Arena(int32_t capacity, Clay_Arena *arena) {  \
    return CLAY__INIT(arrayName){.capacity = capacity,                         \
                                 .length = 0,                                  \
                                 .internalArray =                              \
                                     (typeName *)Clay__Array_Allocate_Arena(   \
  ...
  typeName *arrayName##_GetCheckCapacity(arrayName *array, int32_t index) {    \
    return Clay__Array_RangeCheck(index, array->capacity)                      \
               ? &array->internalArray[index]                                  \
               : &typeName##_DEFAULT;                                          \
  }                                                                            \
                                                                               \
  ...
  typeName *arrayName##_Set(arrayName *array, int32_t index, typeName value) { \
    ...
    return NULL;                                                               \
                                                                               ...
                                                                               \
  typeName *arrayName##_Set_DontTouchLength(arrayName *array, int32_t index,   \
                                            typeName value) {                  \
    if (Clay__Array_RangeCheck(index, array->capacity)) {                      \
      array->internalArray[index] = value;                                     \
      return &array->internalArray[index];                                     \
...
CLAY__ARRAY_DEFINE(Clay__TransitionDataInternal,
                   Clay__TransitionDataInternalArray)
...
typedef struct {
  ...
  int16_t zIndex;
  Clay_Vector2
      pointerOffset; // Only used when scroll containers are managed externally
...
Clay_String Clay__WriteStringToCharBuffer(Clay__charArray *buffer,
  ...
  return CLAY__INIT(Clay_String){.length = string.length,
                                 .chars = (const char *)(buffer->internalArray +
                                                         buffer->length -
                                                         string.length)};
}
...
uint32_t Clay__GetParentElementId(void) { return Clay__GetParentElement()->id; }

bool Clay__BorderHasAnyWidth(Clay_BorderElementConfig *borderConfig) {
  return borderConfig->width.betweenChildren > 0 ||
         borderConfig->width.left > 0 || borderConfig->width.right > 0 ||
...
uint32_t Clay__HashStringContentsWithConfig(Clay_String *text,
  ...
  if (text->isStaticallyAllocated) {
    ...
    hash =
        Clay__HashData((const uint8_t *)text->chars, text->length) % UINT32_MAX;
  }
...
Clay__MeasureTextCached(Clay_String *text, Clay_TextElementConfig *config) {
    ...
    if (!context->booleanWarnings.textMeasurementFunctionNotSet) {
      ...
      context->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData){
          ...
          .errorText = CLAY_STRING(
              "Clay's internal MeasureText function is null. You may have "
              "forgotten to call Clay_SetMeasureTextFunction(), or passed a "
              "NULL function pointer by mistake."),
  ...
  while (elementIndex != 0) {
    ...
    if (context->generation - hashEntry->generation > 2) {
      // Add all the measured words that were included in this measurement to
      // the freelist
      ...
      Clay__MeasureTextCacheItemArray_Set(
          &context->measureTextHashMapInternal, elementIndex,
          CLAY__INIT(Clay__MeasureTextCacheItem){.measuredWordsStartIndex =
                                                     -1});
  ...
  if (context->measureTextHashMapInternalFreeList.length > 0) {
    ...
    Clay__MeasureTextCacheItemArray_Set(&context->measureTextHashMapInternal,
                                        newItemIndex, newCacheItem);
    ...
    if (context->measureTextHashMapInternal.length ==
      ...
      if (!context->booleanWarnings.maxTextMeasureCacheExceeded) {
        context->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData){
            ...
            .errorText =
                CLAY_STRING("Clay ran out of capacity while attempting to "
    ...
    if (context->measuredWords.length == context->measuredWords.capacity - 1) {
      if (!context->booleanWarnings.maxTextMeasureCacheExceeded) {
        context->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData){
            ...
            .errorText = CLAY_STRING(
                "Clay has run out of space in it's internal text measurement "
  ...
  while (hashItemIndex != -1) { // Just replace collision, not a big deal -
    ...
    if (hashItem->elementId.id ==
      ...
      if (hashItem->generation <=
          context->generation) { // First collision - assume this is the "same"
                                 // element
        hashItem->appearedThisFrame =
            hashItem->generation < context->generation;
        hashItem->elementId =
            elementId; // Make sure to copy this across. If the stringId
                       // reference has changed, we should update the hash item
                       // to use the new one.
        ...
        context->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData){
            ...
            .errorText = CLAY_STRING("An element with this ID was already "
  ...
  if (layoutElement->config.aspectRatio.aspectRatio != 0) {
    if (layoutElement->dimensions.width == 0 &&
      ...
      layoutElement->dimensions.width =
          ...
          layoutElement->config.aspectRatio.aspectRatio;
    } else if (layoutElement->dimensions.width != 0 &&
               layoutElement->dimensions.height == 0) {
      layoutElement->dimensions.height =
          layoutElement->dimensions.width *
          (1 / layoutElement->config.aspectRatio.aspectRatio);
    }
  }
}
...
void Clay__CloseElement(void) {
  ...
  if (elementHasClipHorizontal || elementHasClipVertical ||
      openLayoutElement->config.floating.attachTo != CLAY_ATTACH_TO_NONE) {
  ...
  openLayoutElement->children.elements =
  ...
  if (layoutConfig->layoutDirection == CLAY_LEFT_TO_RIGHT) {
    ...
    for (int32_t i = 0; i < openLayoutElement->children.length; i++) {
      int32_t childIndex = Clay__int32_tArray_GetValue(
          ...
          (int)context->layoutElementChildrenBuffer.length -
              openLayoutElement->children.length + i);
      ...
      // Minimum size of child elements doesn't matter to clip containers as
      // they can shrink and hide their contents
    ...
    float childGap =
        (float)(CLAY__MAX(openLayoutElement->children.length - 1, 0) *
  ...
  if (layoutConfig->sizing.width.type != CLAY__SIZING_TYPE_PERCENT) {
    if (layoutConfig->sizing.width.size.minMax.max <=
        0) { // Set the max size if the user didn't specify, makes calculations
             // easier
...
void Clay__OpenElement(void) {
  ...
  Clay_LayoutElement *parentElement = Clay_LayoutElementArray_Get(
      ...
      Clay__int32_tArray_GetValue(&context->openLayoutElementStack,
                                  context->openLayoutElementStack.length - 2));
  uint32_t offset =
      parentElement->children.length + parentElement->floatingChildrenCount;
...
void Clay__OpenTextElement(Clay_String text,
  ...
  Clay_LayoutElement layoutElement = {.textConfig = textConfig,
                                      .isTextElement = true};
  ...
  textElement->textElementData = CLAY__INIT(Clay__TextElementData){
      .text = text, .preferredDimensions = textMeasured->unwrappedDimensions};
...
void Clay__ConfigureOpenElementPtr(const Clay_ElementDeclaration *declaration) {
  ...
  if ((declaration->layout.sizing.width.type == CLAY__SIZING_TYPE_PERCENT &&
    ...
    context->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData){
        ...
        .errorText =
            CLAY_STRING("An element was configured with CLAY_SIZING_PERCENT, "
                        "but the provided percentage value was over 1.0. Clay "
  ...
  if (declaration->floating.attachTo != CLAY_ATTACH_TO_NONE) {
    Clay_FloatingElementConfig *floatingConfig =
        &openLayoutElement->config.floating;
    // This looks dodgy but because of the auto generated root element the depth
    // of the tree will always be at least 2 here
      ...
      if (declaration->floating.attachTo == CLAY_ATTACH_TO_PARENT) {
        ...
        if (parentItem == &Clay_LayoutElementHashMapItem_DEFAULT) {
          context->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData){
              ...
              .errorText = CLAY_STRING(
                  "A floating element was declared with a parentId, but no "
  ...
  if (declaration->transition.handler) {
    ...
    for (int32_t i = 0; i < context->transitionDatas.length; i++) {
      Clay__TransitionDataInternal *existingData =
      ...
      if (openLayoutElement->id == existingData->elementId) {
        ...
        transitionData->transitionOut =
            !!declaration->transition.exit.setFinalState;
      }
    }
    if (!transitionData) {
      transitionData = Clay__TransitionDataInternalArray_Add(
          &context->transitionDatas,
          CLAY__INIT(Clay__TransitionDataInternal){
...
void Clay__InitializePersistentMemory(Clay_Context *context) {
  ...
  context->transitionDatas =
      Clay__TransitionDataInternalArray_Allocate_Arena(200, arena);
...
void Clay__SizeContainersAlongAxis(bool xAxis, float deltaTime,
                                   Clay__int32_tArray *textElementsOut,
                                   Clay__int32_tArray *aspectRatioElementsOut) {
  Clay_Context *context = Clay_GetCurrentContext();
  ...
  for (int32_t rootIndex = 0;
    ...
    bfsBuffer.length = 0;
    Clay__LayoutElementTreeRoot *root = Clay__LayoutElementTreeRootArray_Get(
      ...
      if (parentItem && parentItem != &Clay_LayoutElementHashMapItem_DEFAULT) {
        ...
        case CLAY__SIZING_TYPE_PERCENT: {
          rootElement->dimensions.width =
              ...
              rootElement->config.layout.sizing.width.size.percent;
          break;
        }
        default:
          break;
        ...
        case CLAY__SIZING_TYPE_PERCENT: {
          rootElement->dimensions.height =
              parentLayoutElement->dimensions.height *
              rootElement->config.layout.sizing.height.size.percent;
          break;
        }
        default:
          break;
    ...
    if (rootElement->config.layout.sizing.width.type !=
        CLAY__SIZING_TYPE_PERCENT) {
      rootElement->dimensions.width = CLAY__MIN(
          CLAY__MAX(rootElement->dimensions.width,
                    rootElement->config.layout.sizing.width.size.minMax.min),
          rootElement->config.layout.sizing.width.size.minMax.max);
    }
    if (rootElement->config.layout.sizing.height.type !=
        CLAY__SIZING_TYPE_PERCENT) {
      rootElement->dimensions.height = CLAY__MIN(
          CLAY__MAX(rootElement->dimensions.height,
                    rootElement->config.layout.sizing.height.size.minMax.min),
          rootElement->config.layout.sizing.height.size.minMax.max);
    }
    ...
    for (int32_t i = 0; i < bfsBuffer.length; ++i) {
      ...
      float parentPadding =
          (float)(xAxis ? (parentLayoutConfig->padding.left +
                           parentLayoutConfig->padding.right)
                        : (parentLayoutConfig->padding.top +
                           parentLayoutConfig->padding.bottom));
      float innerContentSize = 0, totalPaddingAndChildGaps = parentPadding;
      bool sizingAlongAxis =
          (xAxis &&
           parentLayoutConfig->layoutDirection == CLAY_LEFT_TO_RIGHT) ||
          (!xAxis && parentLayoutConfig->layoutDirection == CLAY_TOP_TO_BOTTOM);
      resizableContainerBuffer.length = 0;
      float parentChildGap = parentLayoutConfig->childGap;
      bool isFirstChild = true;

      for (int32_t childOffset = 0; childOffset < parent->children.length;
           childOffset++) {
        ...
        Clay_SizingAxis childSizing =
            xAxis ? childElement->config.layout.sizing.width
                  : childElement->config.layout.sizing.height;
        ...
        if (aspectRatioElementsOut &&
            childElement->config.aspectRatio.aspectRatio != 0) {
        ...
        if (sizingAlongAxis) {
          ...
          if (!isFirstChild) {
            innerContentSize += parentChildGap; // For children after index 0,
                                                // the childAxisOffset is the
                                                // gap from the previous child
      ...
      if (sizingAlongAxis) {
        ...
        if (sizeToDistribute < 0) {
          // If the parent clips content in this axis direction, don't compress
          // children, just leave them alone
          ...
          while (sizeToDistribute < -CLAY__EPSILON &&
            ...
            for (int childIndex = 0;
                 childIndex < resizableContainerBuffer.length; childIndex++) {
              Clay_LayoutElement *child = Clay_LayoutElementArray_Get(
                  &context->layoutElements,
                  Clay__int32_tArray_GetValue(&resizableContainerBuffer,
                                              childIndex));
              float childSize =
            ...
            widthToAdd = CLAY__MAX(
                widthToAdd, sizeToDistribute / resizableContainerBuffer.length);
          ...
          for (int childIndex = 0; childIndex < resizableContainerBuffer.length;
            ...
            Clay__SizingType childSizing =
                xAxis ? child->config.layout.sizing.width.type
                      : child->config.layout.sizing.height.type;
          ...
          while (sizeToDistribute > CLAY__EPSILON &&
            ...
            for (int childIndex = 0;
              ...
              float *childSize =
                  xAxis ? &child->dimensions.width : &child->dimensions.height;
              float maxSize =
                  xAxis ? child->config.layout.sizing.width.size.minMax.max
                        : child->config.layout.sizing.height.size.minMax.max;
              float previousWidth = *childSize;
              if (Clay__FloatEqual(*childSize, smallest)) {
                *childSize += widthToAdd;
        ...
        for (int32_t childOffset = 0;
             childOffset < resizableContainerBuffer.length; childOffset++) {
          Clay_LayoutElement *childElement = Clay_LayoutElementArray_Get(
              &context->layoutElements,
              Clay__int32_tArray_GetValue(&resizableContainerBuffer,
                                          childOffset));
          ...
          // If we're laying out the children of a scroll panel, grow containers
          // expand to the size of the inner content, not the outer container
          if (((xAxis && parent->config.clip.horizontal) ||
               (!xAxis && parent->config.clip.vertical))) {
  ...
  if (context->renderCommands.length < context->renderCommands.capacity - 1) {
    ...
    if (!context->booleanWarnings.maxRenderCommandsExceeded) {
      ...
      context->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData){
          .errorType = CLAY_ERROR_TYPE_ELEMENTS_CAPACITY_EXCEEDED,
          .errorText = CLAY_STRING(
              "Clay ran out of capacity while attempting to create render "
              "commands. This is usually caused by a large amount of wrapping "
              "text elements while close to the max element capacity. Try "
...
void Clay__CalculateFinalLayout(float deltaTime, bool useStoredBoundingBoxes,
  ...
  Clay__SizeContainersAlongAxis(true, deltaTime, &textElements,
                                &aspectRatioElements);

  // Wrap text
  for (int32_t textElementIndex = 0; textElementIndex < textElements.length;
       ++textElementIndex) {
    Clay_LayoutElement *element = Clay_LayoutElementArray_Get(
        &context->layoutElements,
        Clay__int32_tArray_GetValue(&textElements, textElementIndex));
    ...
    textElementData->wrappedLines = CLAY__INIT(Clay__WrappedTextLineArraySlice){
        .length = 0,
        .internalArray = &context->wrappedTextLines
                              .internalArray[context->wrappedTextLines.length]};
    Clay_LayoutElement *containerElement = Clay_LayoutElementArray_Get(
        &context->layoutElements,
        Clay__int32_tArray_GetValue(&textElements, textElementIndex));
    Clay__MeasureTextCacheItem *measureTextCacheItem = Clay__MeasureTextCached(
        &textElementData->text, &containerElement->textConfig);
    float lineWidth = 0;
    float lineHeight = containerElement->textConfig.lineHeight > 0
                           ? (float)containerElement->textConfig.lineHeight
    ...
    float spaceWidth =
        Clay__MeasureText(
            CLAY__INIT(Clay_StringSlice){.length = 1,
                                         .chars = CLAY__SPACECHAR.chars,
                                         .baseChars = CLAY__SPACECHAR.chars},
            &containerElement->textConfig, context->measureTextUserData)
            .width;
    ...
    while (wordIndex != -1) {
      ...
      if (lineLengthChars == 0 && lineWidth + measuredWord->width >
                                      containerElement->dimensions.width) {
        Clay__WrappedTextLineArray_Add(
            &context->wrappedTextLines,
      ...
      // measuredWord->length == 0 means a newline character
        ...
        lineWidth +=
            measuredWord->width + containerElement->textConfig.letterSpacing;
    ...
    if (lineLengthChars > 0) {
      Clay__WrappedTextLineArray_Add(
          &context->wrappedTextLines,
          CLAY__INIT(Clay__WrappedTextLine){
              {lineWidth - containerElement->textConfig.letterSpacing,
               lineHeight},
  ...
  for (int32_t i = 0; i < aspectRatioElements.length; ++i) {
    Clay_LayoutElement *aspectElement = Clay_LayoutElementArray_Get(
        &context->layoutElements,
        Clay__int32_tArray_GetValue(&aspectRatioElements, i));
    aspectElement->dimensions.height =
        (1 / aspectElement->config.aspectRatio.aspectRatio) *
        aspectElement->dimensions.width;
    aspectElement->config.layout.sizing.height.size.minMax.max =
        aspectElement->dimensions.height;
  }
  ...
  while (dfsBuffer.length > 0) {
    ...
    if (!context->treeNodeVisited.internalArray[dfsBuffer.length - 1]) {
      ...
      // If the element has no children or is the container for a text element,
      // don't bother inspecting it
      if (currentElement->isTextElement ||
          currentElement->children.length == 0) {
        dfsBuffer.length--;
        continue;
      }
      // Add the children to the DFS buffer (needs to be pushed in reverse so
      // that stack traversal is in correct layout order)
      for (int32_t i = 0; i < currentElement->children.length; i++) {
        ...
        Clay__LayoutElementTreeNodeArray_Add(
            &dfsBuffer, CLAY__INIT(Clay__LayoutElementTreeNode){
                            .layoutElement = Clay_LayoutElementArray_Get(
                                &context->layoutElements,
                                currentElement->children.elements[i])});
      }
      continue;
    }
    dfsBuffer.length--;
    ...
    if (layoutConfig->layoutDirection == CLAY_LEFT_TO_RIGHT) {
      // Resize any parent containers that have grown in height along their non
      // layout axis
      for (int32_t j = 0; j < currentElement->children.length; ++j) {
        Clay_LayoutElement *childElement = Clay_LayoutElementArray_Get(
            &context->layoutElements, currentElement->children.elements[j]);
        float childHeightWithPadding = CLAY__MAX(
            childElement->dimensions.height + layoutConfig->padding.top +
                layoutConfig->padding.bottom,
            currentElement->dimensions.height);
        currentElement->dimensions.height =
            CLAY__MIN(CLAY__MAX(childHeightWithPadding,
                                layoutConfig->sizing.height.size.minMax.min),
                      layoutConfig->sizing.height.size.minMax.max);
      }
    } else if (layoutConfig->layoutDirection == CLAY_TOP_TO_BOTTOM) {
      ...
      contentHeight +=
          (float)(CLAY__MAX(currentElement->children.length - 1, 0) *
                  layoutConfig->childGap);
  ...
  for (int32_t i = 0; i < aspectRatioElements.length; ++i) {
    Clay_LayoutElement *aspectElement = Clay_LayoutElementArray_Get(
        ...
        Clay__int32_tArray_GetValue(&aspectRatioElements, i));
    aspectElement->dimensions.width =
        aspectElement->config.aspectRatio.aspectRatio *
        aspectElement->dimensions.height;
  }
    ...
    for (int32_t i = 0; i < sortMax; ++i) {
      Clay__LayoutElementTreeRoot current =
          *Clay__LayoutElementTreeRootArray_Get(
              &context->layoutElementTreeRoots, i);
      Clay__LayoutElementTreeRoot next = *Clay__LayoutElementTreeRootArray_Get(
          &context->layoutElementTreeRoots, i + 1);
      if (next.zIndex < current.zIndex) {
        Clay__LayoutElementTreeRootArray_Set(&context->layoutElementTreeRoots,
                                             i, next);
        Clay__LayoutElementTreeRootArray_Set(&context->layoutElementTreeRoots,
                                             i + 1, current);
      }
  ...
  for (int32_t rootIndex = 0;
    ...
    if (root->clipElementId) {
      ...
      if (clipHashMapItem &&
          !Clay__ElementIsOffscreen(&clipHashMapItem->boundingBox)) {
        // Floating elements that are attached to scrolling contents won't be
        // correctly positioned if external scroll handling is enabled, fix here
        if (context->externalScrollHandlingEnabled) {
          if (clipHashMapItem->layoutElement->config.clip.horizontal) {
            rootPosition.x +=
                clipHashMapItem->layoutElement->config.clip.childOffset.x;
          }
          if (clipHashMapItem->layoutElement->config.clip.vertical) {
            rootPosition.y +=
                clipHashMapItem->layoutElement->config.clip.childOffset.y;
          }
        }
        if (generateRenderCommands) {
          Clay__AddRenderCommand(CLAY__INIT(Clay_RenderCommand){
              ...
              .id = Clay__HashNumber(rootElement->id,
                                     rootElement->children.length + 10)
                        .id, // TODO need a better strategy for managing derived
                             // ids
              .zIndex = root->zIndex,
    ...
    Clay__LayoutElementTreeNodeArray_Add(
        ...
        CLAY__INIT(Clay__LayoutElementTreeNode){
            ...
            .nextChildOffset = {
                .x = (float)rootElement->config.layout.padding.left,
                .y = (float)rootElement->config.layout.padding.top}});
    ...
    while (dfsBuffer.length > 0) {
      ...
      if (context->treeNodeVisited.internalArray[dfsBuffer.length - 1]) {
        ...
        if (generateRenderCommands &&
          ...
          if (currentElement->config.clip.horizontal ||
              currentElement->config.clip.vertical) {
          ...
          if (Clay__BorderHasAnyWidth(&currentElement->config.border)) {
            ...
            Clay_BorderElementConfig *borderConfig =
                &currentElement->config.border;
            Clay_RenderCommand renderCommand = {
                .boundingBox = currentElementBoundingBox,
                .renderData = {.border = {.color = borderConfig->color,
                                          .cornerRadius = currentElement->config
                                                              .cornerRadius,
          ...
          if (closeClipElement) {
            Clay__AddRenderCommand(CLAY__INIT(Clay_RenderCommand){
                .id = Clay__HashNumber(currentElement->id,
                                       rootElement->children.length + 11)
                          .id,
      ...
      // This will only be run a single time for each element in downwards DFS
      // order
      context->treeNodeVisited.internalArray[dfsBuffer.length - 1] = true;
      Clay_BoundingBox currentElementBoundingBox = {
      ...
      if (useStoredBoundingBoxes && currentElement->config.transition.handler) {
        ...
        // An exiting element that completed its transition this frame - skip
        // tree
      ...
      if (currentElement->config.floating.attachTo != CLAY_ATTACH_TO_NONE) {
        Clay_FloatingElementConfig *floatingElementConfig =
            &currentElement->config.floating;
      ...
      if (currentElement->config.clip.horizontal ||
          currentElement->config.clip.vertical) {
        // This linear scan could theoretically be slow under very strange
        // conditions, but I can't imagine a real UI with more than a few 10's
      ...
      if (generateRenderCommands && !offscreen) {
        if (currentElement->isTextElement) {
          Clay_TextElementConfig *textElementConfig =
              &currentElement->textConfig;
          float naturalLineHeight =
              currentElement->textElementData.preferredDimensions.height;
          ...
          for (int32_t lineIndex = 0;
               lineIndex < currentElement->textElementData.wrappedLines.length;
               ++lineIndex) {
            Clay__WrappedTextLine *wrappedLine =
                Clay__WrappedTextLineArraySlice_Get(
                    &currentElement->textElementData.wrappedLines, lineIndex);
                    ...
                    {.text =
                         {
                             .stringContents =
                                 CLAY__INIT(Clay_StringSlice){
                                     ...
                                     .baseChars =
                                         currentElement->textElementData.text
                                             .chars},
          ...
          if (currentElement->config.overlayColor.a > 0) {
            Clay_RenderCommand renderCommand = {
                .renderData = {.overlayColor = {.color = currentElement->config
                                                             .overlayColor}},
                .userData = currentElement->config.userData,
                .id = currentElement->id,
                .zIndex = root->zIndex,
                    ...
                    {.image =
                         {
                             ...
                             .imageData =
                                 currentElement->config.image.imageData,
                         }},
                .userData = currentElement->config.userData,
                .id = currentElement->id,
                .zIndex = root->zIndex,
                    ...
                    {.custom =
                         {
                             ...
                             .cornerRadius =
                                 currentElement->config.cornerRadius,
                             .customData =
                                 currentElement->config.custom.customData,
                         }},
                .userData = currentElement->config.userData,
                .id = currentElement->id,
                .zIndex = root->zIndex,
            ...
            Clay_RenderCommand renderCommand = {
                ...
                .renderData =
                    {.clip =
                         {
                             .horizontal =
                                 currentElement->config.clip.horizontal,
          ...
          if (currentElement->config.backgroundColor.a > 0) {
            Clay_RenderCommand renderCommand = {
                ...
                .renderData = {.rectangle =
                                   {
                                       .backgroundColor = currentElement->config
                                                              .backgroundColor,
                                       .cornerRadius =
                                           currentElement->config.cornerRadius,
                                   }},
      ...
      if (layoutConfig->layoutDirection == CLAY_LEFT_TO_RIGHT) {
        for (int32_t i = 0; i < currentElement->children.length; ++i) {
          Clay_LayoutElement *childElement = Clay_LayoutElementArray_Get(
              &context->layoutElements, currentElement->children.elements[i]);
          if (childElement->exiting)
            continue;
          contentSizeCurrent.width += childElement->dimensions.width;
          contentSizeCurrent.height = CLAY__MAX(
      ...
      if (scrollContainerData) {
        scrollContainerData->contentSize = CLAY__INIT(Clay_Dimensions){
            contentSizeCurrent.width + (float)(layoutConfig->padding.left +
                                               layoutConfig->padding.right),
            contentSizeCurrent.height + (float)(layoutConfig->padding.top +
                                                layoutConfig->padding.bottom)};
      }
      ...
      for (int32_t i = 0; i < currentElement->children.length; ++i) {
        Clay_LayoutElement *childElement = Clay_LayoutElementArray_Get(
            &context->layoutElements, currentElement->children.elements[i]);
        Clay_LayoutElementHashMapItem *childMapItem =
            Clay__GetHashMapItem(childElement->id);
        // Alignment along non layout axis
        if (layoutConfig->layoutDirection == CLAY_LEFT_TO_RIGHT) {
          currentElementTreeNode->nextChildOffset.y =
              currentElement->config.layout.padding.top;
          float whiteSpaceAroundChild = currentElement->dimensions.height -
          ...
          currentElementTreeNode->nextChildOffset.x =
              currentElement->config.layout.padding.left;
          float whiteSpaceAroundChild = currentElement->dimensions.width -
        ...
        // DFS buffer elements need to be added in reverse because stack
        // traversal happens backwards
        uint32_t newNodeIndex = dfsBuffer.length - 1 - i;
        dfsBuffer.internalArray[newNodeIndex] =
            CLAY__INIT(Clay__LayoutElementTreeNode){
                .layoutElement = childElement,
                .position =
                    CLAY__INIT(Clay_Vector2){childPosition.x, childPosition.y},
                .nextChildOffset =
                    {.x = (float)childElement->config.layout.padding.left,
                     .y = (float)childElement->config.layout.padding.top},
            };
...
Clay__DebugElementConfigTypeLabelConfig
Clay__DebugGetElementConfigTypeLabel(Clay__DebugElementConfigType type) {
  switch (type) {
  case CLAY__ELEMENT_CONFIG_TYPE_BACKGROUND_COLOR:
    return CLAY__INIT(Clay__DebugElementConfigTypeLabelConfig){
        CLAY_STRING("Background"), {243, 134, 48, 255}};
  case CLAY__ELEMENT_CONFIG_TYPE_OVERLAY_COLOR:
    return CLAY__INIT(Clay__DebugElementConfigTypeLabelConfig){
        CLAY_STRING("Overlay"), {142, 129, 206, 255}};
  case CLAY__ELEMENT_CONFIG_TYPE_CORNER_RADIUS:
    return CLAY__INIT(Clay__DebugElementConfigTypeLabelConfig){
        CLAY_STRING("Radius"), {239, 148, 157, 255}};
...
void Clay__RenderElementConfigTypeLabel(Clay_String label, Clay_Color color,
                                        bool offscreen) {
...
Clay__RenderDebugLayoutElementsList(int32_t initialRootsLength,
  ...
  for (int32_t rootIndex = 0; rootIndex < initialRootsLength; ++rootIndex) {
    ...
    while (dfsBuffer.length > 0) {
      ...
      if (currentElement->exiting) { // TODO there is a duplicate ID problem
                                     // with exiting elements
        dfsBuffer.length--;
        continue;
      }
      ...
      CLAY(CLAY_IDI("Clay__DebugView_ElementOuter", currentElement->id),
        ...
        if (currentElementData->elementId.stringId.length > 0) {
          CLAY_AUTO_ID() {
            Clay_TextElementConfig textConfig =
                offscreen
                    ? CLAY__INIT(
                          Clay_TextElementConfig){.textColor =
            ...
            if (currentElementData->elementId.offset != 0) {
              ...
              CLAY_TEXT(Clay__IntToString(currentElementData->elementId.offset),
                        textConfig);
              CLAY_TEXT(CLAY_STRING(")"), textConfig);
            }
          }
        ...
        if (currentElement->isTextElement) {
          Clay__RenderElementConfigTypeLabel(
              CLAY_STRING("Text"), CLAY__INIT(Clay_Color){105, 210, 231, 255},
              offscreen);
        } else {
          if (currentElement->config.backgroundColor.a > 0) {
            Clay__DebugElementConfigTypeLabelConfig config =
                Clay__DebugGetElementConfigTypeLabel(
                    CLAY__ELEMENT_CONFIG_TYPE_BACKGROUND_COLOR);
            Clay__RenderElementConfigTypeLabel(config.label, config.color,
                                               offscreen);
          }
          if (currentElement->config.overlayColor.a > 0) {
            Clay__DebugElementConfigTypeLabelConfig config =
          ...
          if (!Clay__MemCmp((const char *)&currentElement->config.cornerRadius,
                            (const char *)&Clay__CornerRadius_DEFAULT,
      ...
      if (currentElement->isTextElement) {
        layoutData.rowCount++;
        Clay__TextElementData *textElementData =
            &currentElement->textElementData;
        Clay_TextElementConfig rawTextConfig =
            offscreen ? CLAY__INIT(
                            Clay_TextElementConfig){.textColor =
                                                        CLAY__DEBUGVIEW_COLOR_3,
                                                    .fontSize = 16}
                      : Clay__DebugView_TextNameConfig;
        CLAY_AUTO_ID(
            {.layout = {.sizing = {.height = CLAY_SIZING_FIXED(
          ...
          CLAY_TEXT(
              textElementData->text.length > 40
                  ? (CLAY__INIT(Clay_String){
                        .length = 40, .chars = textElementData->text.chars})
                  : textElementData->text,
              rawTextConfig);
...
void Clay__DebugViewRenderElementConfigHeader(
    Clay_String elementId, Clay__DebugElementConfigType type) {
  Clay__DebugElementConfigTypeLabelConfig config =
...
void Clay__RenderDebugView(void) {
  ...
  CLAY(CLAY_ID("Clay__DebugView"),
    ...
    CLAY_AUTO_ID(
      ...
      CLAY_AUTO_ID({
          .layout =
              {.sizing = {CLAY_SIZING_FIXED(CLAY__DEBUGVIEW_ROW_HEIGHT - 10),
    ...
    CLAY(scrollId,
      ...
      CLAY_AUTO_ID(
        ...
        float contentWidth = Clay__GetHashMapItem(panelContentsId.id)
                                 ->layoutElement->dimensions.width;
        CLAY_AUTO_ID(
            {.layout = {.sizing = {.width = CLAY_SIZING_FIXED(contentWidth)},
                        .layoutDirection = CLAY_TOP_TO_BOTTOM}}) {}
    ...
    if (context->debugSelectedElementId != 0) {
      ...
      CLAY_AUTO_ID(
        ...
        Clay_Padding attributeConfigPadding = {
            CLAY__DEBUGVIEW_OUTER_PADDING, CLAY__DEBUGVIEW_OUTER_PADDING, 8, 8};
        ...
        CLAY_AUTO_ID({.layout = {.padding = attributeConfigPadding,
          ...
          CLAY_AUTO_ID({.layout = {.layoutDirection = CLAY_LEFT_TO_RIGHT}}) {
            ...
            CLAY_TEXT(Clay__IntToString(selectedItem->boundingBox.x),
                      infoTextConfig);
            CLAY_TEXT(CLAY_STRING(", y: "), infoTextConfig);
            CLAY_TEXT(Clay__IntToString(selectedItem->boundingBox.y),
                      infoTextConfig);
          ...
          Clay_LayoutConfig *layoutConfig =
              &selectedItem->layoutElement->config.layout;
          ...
          CLAY_AUTO_ID({.layout = {.layoutDirection = CLAY_LEFT_TO_RIGHT}}) {
            CLAY_TEXT(CLAY_STRING("width: "), infoTextConfig);
            Clay__RenderDebugLayoutSizing(layoutConfig->sizing.width,
                                          infoTextConfig);
          }
          CLAY_AUTO_ID({.layout = {.layoutDirection = CLAY_LEFT_TO_RIGHT}}) {
            CLAY_TEXT(CLAY_STRING("height: "), infoTextConfig);
            Clay__RenderDebugLayoutSizing(layoutConfig->sizing.height,
                                          infoTextConfig);
          }
          // .padding
          ...
          CLAY(CLAY_ID("Clay__DebugViewElementInfoPadding"), {}) {
            ...
            CLAY_TEXT(Clay__IntToString(layoutConfig->padding.right),
                      infoTextConfig);
            ...
            CLAY_TEXT(Clay__IntToString(layoutConfig->padding.bottom),
                      infoTextConfig);
            CLAY_TEXT(CLAY_STRING(" }"), infoTextConfig);
          }
          // .childGap
        ...
        if (selectedItem->layoutElement->isTextElement) {
          Clay_TextElementConfig *textConfig =
              &selectedItem->layoutElement->textConfig;
          CLAY_AUTO_ID({.layout = {.padding = attributeConfigPadding,
                                   .childGap = 8,
                                   .layoutDirection = CLAY_TOP_TO_BOTTOM}}) {
            Clay__DebugViewRenderElementConfigHeader(
                selectedItem->elementId.stringId,
                CLAY__ELEMENT_CONFIG_TYPE_TEXT);
            // .fontSize
            ...
            CLAY_TEXT(Clay__IntToString(textConfig->letterSpacing),
                      infoTextConfig);
            // .wrapMode
          ...
          CLAY(CLAY_ID("Clay__DebugViewElementInfoSharedBody"),
               {.layout = {.padding = attributeConfigPadding,
                           .childGap = 8,
                           .layoutDirection = CLAY_TOP_TO_BOTTOM}}) {
            Clay__DebugElementConfigTypeLabelConfig labelConfig =
                Clay__DebugGetElementConfigTypeLabel(
                    CLAY__ELEMENT_CONFIG_TYPE_BACKGROUND_COLOR);
            ...
            CLAY_AUTO_ID({.layout = {.padding = {8, 8, 2, 2}},
                          .backgroundColor = backgroundColor,
                          .cornerRadius = CLAY_CORNER_RADIUS(4),
                          .border = {.color = labelConfig.color,
                                     .width = {1, 1, 1, 1, 0}}}) {
              CLAY_TEXT(CLAY_STRING("Color & Radius"),
                        CLAY_TEXT_CONFIG({.textColor = CLAY__DEBUGVIEW_COLOR_4,
                                          .fontSize = 16}));
            }
            // .backgroundColor
            if (selectedItem->layoutElement->config.backgroundColor.a > 0) {
              ...
              Clay__RenderDebugViewColor(
                  selectedItem->layoutElement->config.backgroundColor,
                  infoTextConfig);
            }
            // .cornerRadius
            if (!Clay__MemCmp((const char *)&selectedItem->layoutElement->config
                                  .cornerRadius,
                              (const char *)&Clay__CornerRadius_DEFAULT,
                              sizeof(Clay_CornerRadius))) {
              CLAY_TEXT(CLAY_STRING("Corner Radius"), infoTitleConfig);
              Clay__RenderDebugViewCornerRadius(
                  selectedItem->layoutElement->config.cornerRadius,
                  infoTextConfig);
            }
            // .overlayColor
          ...
          if (selectedItem->layoutElement->config.image.imageData) {
            Clay_ImageElementConfig *imageConfig =
                &selectedItem->layoutElement->config.image;
            Clay_AspectRatioElementConfig aspectConfig = {1};
            if (selectedItem->layoutElement->config.aspectRatio.aspectRatio >
                0) {
          ...
          if (selectedItem->layoutElement->config.floating.attachTo !=
              CLAY_ATTACH_TO_NONE) {
            Clay_FloatingElementConfig *floatingConfig =
                &selectedItem->layoutElement->config.floating;
            CLAY_AUTO_ID({.layout = {.padding = attributeConfigPadding,
                                     .childGap = 8,
                                     .layoutDirection = CLAY_TOP_TO_BOTTOM}}) {
              Clay__DebugViewRenderElementConfigHeader(
          ...
          Clay_ClipElementConfig *clipConfig =
              &selectedItem->layoutElement->config.clip;
          ...
          Clay_BorderElementConfig *borderConfig =
              &selectedItem->layoutElement->config.border;
...
Clay__WarningArray Clay__WarningArray_Allocate_Arena(int32_t capacity,
  ...
  if (nextAllocOffset + totalSizeBytes <= arena->capacity) {
    ...
    Clay__currentContext->errorHandler.errorHandlerFunction(CLAY__INIT(
        Clay_ErrorData){
        .errorType = CLAY_ERROR_TYPE_ARENA_CAPACITY_EXCEEDED,
        .errorText =
            CLAY_STRING("Clay attempted to allocate memory in its arena, but "
                        "ran out of capacity. Try increasing the capacity of "
                        "the arena passed to Clay_Initialize()"),
...
void Clay_SetLayoutDimensions(Clay_Dimensions dimensions) {
  ...
  context->rootResizedLastFrame =
      !Clay__FloatEqual(context->layoutDimensions.width, dimensions.width) ||
      !Clay__FloatEqual(context->layoutDimensions.height, dimensions.height);
  ...
  for (int32_t rootIndex = context->layoutElementTreeRoots.length - 1;
    ...
    while (dfsBuffer.length > 0) {
      ...
      Clay_LayoutElement *currentElement = Clay_LayoutElementArray_Get(
          &context->layoutElements,
          Clay__int32_tArray_GetValue(&dfsBuffer, (int)dfsBuffer.length - 1));
      // Skip mouse interactions on an element if it's currently transitioning,
      ...
      if (currentElement->config.transition.handler) {
        for (int I = 0; I < context->transitionDatas.length; ++I) {
          ...
          if (data->elementId == currentElement->id) {
            if (currentElement->config.transition.interactionHandling ==
                CLAY_TRANSITION_DISABLE_INTERACTIONS_WHILE_TRANSITIONING_POSITION) {
              if (data->state == CLAY_TRANSITION_STATE_EXITING ||
                  data->state == CLAY_TRANSITION_STATE_ENTERING ||
                  ((data->activeProperties &
                    CLAY_TRANSITION_PROPERTY_POSITION) &&
                   data->state == CLAY_TRANSITION_STATE_TRANSITIONING)) {
                skipTree = true;
              }
            } else if (
                currentElement->config.transition.interactionHandling ==
                CLAY_TRANSITION_ALLOW_INTERACTIONS_WHILE_TRANSITIONING_POSITION) {
              if (data->state == CLAY_TRANSITION_STATE_EXITING) {
                skipTree = true;
              }
      ...
      Clay_LayoutElementHashMapItem *mapItem = Clay__GetHashMapItem(
          currentElement
              ->id); // TODO think of a way around this, maybe the fact that
                     // it's essentially a binary tree limits the cost, but the
                     // worst case is not great
    ...
    if (found && rootElement->config.floating.attachTo != CLAY_ATTACH_TO_NONE &&
        rootElement->config.floating.pointerCaptureMode ==
            CLAY_POINTER_CAPTURE_MODE_CAPTURE) {
      break;
    }
  }
...
void Clay_UpdateScrollContainers(bool enableDragScrolling,
  ...
  for (int32_t i = 0; i < context->scrollContainerDatas.length; i++) {
    ...
    scrollData->scrollPosition.x = CLAY__MIN(
        CLAY__MAX(scrollData->scrollPosition.x,
    ...
    scrollData->scrollPosition.y = CLAY__MIN(
        CLAY__MAX(scrollData->scrollPosition.y,
  ...
  if (highestPriorityElementIndex > -1 && highestPriorityScrollData) {
    ...
    bool canScrollVertically =
        clipConfig->vertical && highestPriorityScrollData->contentSize.height >
                                    scrollElement->dimensions.height;
    bool canScrollHorizontally =
        clipConfig->horizontal && highestPriorityScrollData->contentSize.width >
    ...
    if (isPointerActive) {
      ...
      if (!highestPriorityScrollData->pointerScrollActive) {
        ...
        if (canScrollHorizontally) {
          ...
          scrollDeltaX =
              highestPriorityScrollData->scrollPosition.x - oldXScrollPosition;
        }
        if (canScrollVertically) {
          ...
          scrollDeltaY =
              highestPriorityScrollData->scrollPosition.y - oldYScrollPosition;
        }
...
void Clay__CloneElementsWithExitTransition() {
  ...
  for (int i = 0; i < context->transitionDatas.length; ++i) {
    Clay__TransitionDataInternal *data =
        Clay__TransitionDataInternalArray_Get(&context->transitionDatas, i);
    Clay_TransitionElementConfig *config =
        &data->elementThisFrame->config.transition;
    if (data->transitionOut) {
      Clay__int32_tArray bfsBuffer = context->openLayoutElementStack;
      bfsBuffer.length = 0;
      Clay_LayoutElement *newElement =
          Clay_LayoutElementArray_Set_DontTouchLength(
              &context->layoutElements, nextIndex, *data->elementThisFrame);
      Clay__StringArray_Set_DontTouchLength(
          &context->layoutElementIdStrings, nextIndex,
          *Clay__StringArray_GetCheckCapacity(
              &context->layoutElementIdStrings,
              data->elementThisFrame - context->layoutElements.internalArray));
      ...
      while (bufferIndex < bfsBuffer.length) {
        Clay_LayoutElement *layoutElement =
            Clay_LayoutElementArray_GetCheckCapacity(
                &context->layoutElements,
                Clay__int32_tArray_GetValue(&bfsBuffer, bufferIndex));
        bufferIndex++;
        for (int j = layoutElement->children.length - 1; j >= 0; --j) {
          Clay_LayoutElement *childElement =
              Clay_LayoutElementArray_GetCheckCapacity(
                  &context->layoutElements,
                  layoutElement->children.elements[j]);
          Clay__int32_tArray_Add(&bfsBuffer, nextIndex);
          Clay_LayoutElement *newChildElement =
              Clay_LayoutElementArray_Set_DontTouchLength(
                  &context->layoutElements, nextIndex, *childElement);
          Clay__StringArray_Set_DontTouchLength(
              &context->layoutElementIdStrings, nextIndex,
              *Clay__StringArray_GetCheckCapacity(
                  &context->layoutElementIdStrings,
                  childElement - context->layoutElements.internalArray));
          Clay__int32_tArray_Set_DontTouchLength(
              &context->layoutElementChildren, nextChildIndex, nextIndex);
          nextIndex--;
          nextChildIndex--;
        }
        layoutElement->children.elements =
            &context->layoutElementChildren.internalArray[nextChildIndex + 1];
      }
    }
  }
};

void Clay_ApplyTransitionedPropertiesToElement(
    Clay_LayoutElement *currentElement, Clay_TransitionProperty properties,
    Clay_TransitionData currentTransitionData, Clay_BoundingBox *boundingBox,
    bool reparented) {
  if (properties & CLAY_TRANSITION_PROPERTY_WIDTH) {
    if (!reparented) {
      currentElement->dimensions.width =
          currentTransitionData.boundingBox.width;
      currentElement->config.layout.sizing.width =
          CLAY_SIZING_FIXED(currentTransitionData.boundingBox.width);
    } else {
  ...
  if (properties & CLAY_TRANSITION_PROPERTY_HEIGHT) {
    if (!reparented) {
      currentElement->dimensions.height =
          currentTransitionData.boundingBox.height;
      currentElement->config.layout.sizing.height =
          CLAY_SIZING_FIXED(currentTransitionData.boundingBox.height);
    } else {
  ...
  if (properties & CLAY_TRANSITION_PROPERTY_BACKGROUND_COLOR) {
    currentElement->config.backgroundColor =
        currentTransitionData.backgroundColor;
  }
...
Clay_RenderCommandArray Clay_EndLayout(float deltaTime) {
  ...
  for (int i = 0; i < context->transitionDatas.length; ++i) {
    Clay__TransitionDataInternal *data =
        Clay__TransitionDataInternalArray_Get(&context->transitionDatas, i);
    Clay_LayoutElementHashMapItem *hashMapItem =
        Clay__GetHashMapItem(data->elementId);
    // This might seems strange - can't we just look up the element itself, and
    // check the config to see whether it has an exit transition defined? That
    // would work fine if the element actually had an exit transition in the
    // first place. If it doesn't have an exit transition defined, the element
    // will have simply disappeared completely at this point, and there will be
    // no element through which to access the config.
    if (data->transitionOut) {
      Clay_TransitionElementConfig *config =
          &data->elementThisFrame->config.transition;
      // Element wasn't found this frame - either delete transition data or
      // transition out
      if (hashMapItem->generation <= context->generation) {
        Clay_LayoutElementHashMapItem *parentHashMapItem =
            Clay__GetHashMapItem(data->parentId);
        // Don't exit transition if the parent has also exited and
        // SKIP_WHEN_PARENT_EXITS is used
        if (config->exit.trigger ==
                CLAY_TRANSITION_EXIT_TRIGGER_WHEN_PARENT_EXITS ||
            !parentHashMapItem ||
            parentHashMapItem->generation > context->generation) {
          if (data->state != CLAY_TRANSITION_STATE_EXITING) {
            if (parentHashMapItem->generation <= context->generation) {
              data->elementThisFrame->config.floating.attachTo =
                  CLAY_ATTACH_TO_ROOT;
              data->elementThisFrame->config.floating.offset =
                  CLAY__INIT(Clay_Vector2){hashMapItem->boundingBox.x,
                                           hashMapItem->boundingBox.y};
            }
            data->elementThisFrame->exiting = true;
            data->elementThisFrame->config.layout.sizing.width =
                CLAY_SIZING_FIXED(data->elementThisFrame->dimensions.width);
            data->elementThisFrame->config.layout.sizing.height =
                CLAY_SIZING_FIXED(data->elementThisFrame->dimensions.height);
          ...
          Clay__int32_tArray bfsBuffer = context->openLayoutElementStack;
          bfsBuffer.length = 0;
          data->elementThisFrame = Clay_LayoutElementArray_Add(
              &context->layoutElements, *data->elementThisFrame);
          int32_t exitingElementIndex =
              data->elementThisFrame - context->layoutElements.internalArray;
          Clay__StringArray_Add(
              &context->layoutElementIdStrings,
              *Clay__StringArray_GetCheckCapacity(
                  &context->layoutElementIdStrings, exitingElementIndex));
          Clay__int32_tArray_Add(
              &context->layoutElementClipElementIds,
              *Clay__int32_tArray_GetCheckCapacity(
                  &context->layoutElementClipElementIds, exitingElementIndex));
          ...
          while (bufferIndex < bfsBuffer.length) {
            ...
            for (int j = 0; j < layoutElement->children.length; ++j) {
              ...
              int32_t childElementIndex =
                  childElement - context->layoutElements.internalArray;
              Clay_LayoutElement *newChildElement = Clay_LayoutElementArray_Add(
                  &context->layoutElements, *childElement);
              Clay__StringArray_Add(
                  &context->layoutElementIdStrings,
                  *Clay__StringArray_GetCheckCapacity(
                      &context->layoutElementIdStrings, childElementIndex));
              Clay__int32_tArray_Add(&context->layoutElementClipElementIds,
                                     *Clay__int32_tArray_GetCheckCapacity(
                                         &context->layoutElementClipElementIds,
                                         childElementIndex));
              ...
              Clay__int32_tArray_Add(&context->layoutElementChildren,
                                     context->layoutElements.length - 1);
            }
            layoutElement->children.elements =
                &context->layoutElementChildren.internalArray[firstChildSlot];
          }

          // Reattach the inserted subtree to its previous parent if it still
          // exists
          if (parentHashMapItem->generation > context->generation) {
            Clay_LayoutElement *parentElement =
                parentHashMapItem->layoutElement;
            int32_t newChildrenStartIndex =
                context->layoutElementChildren.length;
            bool found = false;
            if (config->exit.siblingOrdering ==
                CLAY_EXIT_TRANSITION_ORDERING_UNDERNEATH_SIBLINGS) {
              Clay__int32_tArray_Add(&context->layoutElementChildren,
                                     exitingElementIndex);
              found = true;
            ...
            for (int j = 0; j < parentElement->children.length; ++j) {
              ...
              Clay__int32_tArray_Add(&context->layoutElementChildren,
                                     parentElement->children.elements[j]);
            }
            if (!found) {
              Clay__int32_tArray_Add(&context->layoutElementChildren,
                                     exitingElementIndex);
            ...
            parentElement->children.length++;
            parentElement->children.elements =
                &context->layoutElementChildren
                     .internalArray[newChildrenStartIndex];
            ...
            Clay__LayoutElementTreeRootArray_Add(
                ...
                CLAY__INIT(Clay__LayoutElementTreeRoot){
                    .layoutElementIndex =
                        (int32_t)(data->elementThisFrame -
                                  context->layoutElements.internalArray),
                    .parentId =
                        Clay__HashString(CLAY_STRING("Clay__RootContainer"), 0)
                            .id,
                    .zIndex = 1,
                });
          }
  ...
  if (context->booleanWarnings.maxElementsExceeded) {
    ...
    if (context->transitionDatas.length > 0) {
      ...
      for (int i = 0; i < context->transitionDatas.length; ++i) {
        ...
        Clay_LayoutElementHashMapItem *mapItem =
            Clay__GetHashMapItem(transitionData->elementId);
        Clay_LayoutElementHashMapItem *parentMapItem =
            Clay__GetHashMapItem(transitionData->parentId);
        ...
        if (mapItem->appearedThisFrame) {
          if (currentElement->config.transition.enter.setInitialState &&
              !(parentMapItem->appearedThisFrame &&
                currentElement->config.transition.enter.trigger ==
                    CLAY_TRANSITION_ENTER_SKIP_ON_FIRST_PARENT_FRAME)) {
            transitionData->state = CLAY_TRANSITION_STATE_ENTERING;
            transitionData->initialState =
                currentElement->config.transition.enter.setInitialState(
            ...
            Clay_ApplyTransitionedPropertiesToElement(
                currentElement, currentElement->config.transition.properties,
                transitionData->initialState, &mapItem->boundingBox,
          ...
          Clay_Vector2 parentScrollOffset =
              parentMapItem->layoutElement->config.clip.childOffset;
          Clay_Vector2 newRelativePosition = {
              mapItem->boundingBox.x - parentMapItem->boundingBox.x -
                  parentScrollOffset.x,
              mapItem->boundingBox.y - parentMapItem->boundingBox.y -
                  parentScrollOffset.y,
          };
          Clay_Vector2 oldRelativePosition =
              transitionData->oldParentRelativePosition;
          transitionData->oldParentRelativePosition = newRelativePosition;
          Clay_TransitionProperty properties =
              currentElement->config.transition.properties;
          int32_t activeProperties = CLAY_TRANSITION_PROPERTY_NONE;
          if (properties & CLAY_TRANSITION_PROPERTY_X) {
            if (!Clay__FloatEqual(oldTargetState.boundingBox.x,
                                  targetState.boundingBox.x) &&
                !(Clay__FloatEqual(oldRelativePosition.x,
                                   newRelativePosition.x)) &&
                !context->rootResizedLastFrame) {
              activeProperties |= CLAY_TRANSITION_PROPERTY_X;
            }
          }
          if (properties & CLAY_TRANSITION_PROPERTY_Y) {
            if (!Clay__FloatEqual(oldTargetState.boundingBox.y,
                                  targetState.boundingBox.y) &&
                !(Clay__FloatEqual(oldRelativePosition.y,
                                   newRelativePosition.y)) &&
                !context->rootResizedLastFrame) {
              activeProperties |= CLAY_TRANSITION_PROPERTY_Y;
            }
          }
          if (properties & CLAY_TRANSITION_PROPERTY_WIDTH) {
            if (!Clay__FloatEqual(oldTargetState.boundingBox.width,
          ...
          if (properties & CLAY_TRANSITION_PROPERTY_HEIGHT) {
            if (!Clay__FloatEqual(oldTargetState.boundingBox.height,
          ...
          if (properties & CLAY_TRANSITION_PROPERTY_BACKGROUND_COLOR) {
            if (!Clay__MemCmp((char *)&oldTargetState.backgroundColor,
                              (char *)&targetState.backgroundColor,
                              sizeof(Clay_Color))) {
              activeProperties |= CLAY_TRANSITION_PROPERTY_BACKGROUND_COLOR;
            }
          }
          if (properties & CLAY_TRANSITION_PROPERTY_OVERLAY_COLOR) {
            if (!Clay__MemCmp((char *)&oldTargetState.overlayColor,
                              (char *)&targetState.overlayColor,
                              sizeof(Clay_Color))) {
              activeProperties |= CLAY_TRANSITION_PROPERTY_OVERLAY_COLOR;
            }
          }
          if (properties & CLAY_TRANSITION_PROPERTY_BORDER_COLOR) {
            if (!Clay__MemCmp((char *)&oldTargetState.borderColor,
                              (char *)&targetState.borderColor,
                              sizeof(Clay_Color))) {
              activeProperties |= CLAY_TRANSITION_PROPERTY_BORDER_COLOR;
            }
          }
          if (properties & CLAY_TRANSITION_PROPERTY_BORDER_WIDTH) {
            if (!Clay__MemCmp((char *)&oldTargetState.borderWidth,
                              (char *)&targetState.borderWidth,
          ...
          if (activeProperties != 0 &&
              transitionData->state != CLAY_TRANSITION_STATE_EXITING) {
          ...
          if (transitionData->state == CLAY_TRANSITION_STATE_IDLE) {
            ...
            transitionComplete = currentElement->config.transition.handler(
                CLAY__INIT(Clay_TransitionCallbackArguments){
                    transitionData->state, transitionData->initialState,
                    &transitionData->currentState, targetState,
            ...
            Clay_ApplyTransitionedPropertiesToElement(
                currentElement, currentElement->config.transition.properties,
                transitionData->currentState, &mapItem->boundingBox,
                transitionData->reparented);
      ...
      if (context->booleanWarnings.maxElementsExceeded) {
        Clay_String message;
        message = CLAY_STRING("Clay Error: Debug view caused layout element "
                              "count to exceed Clay__maxElementCount");
        Clay__AddRenderCommand(CLAY__INIT(Clay_RenderCommand){
            .boundingBox = {context->layoutDimensions.width / 2 - 59 * 4,
                            context->layoutDimensions.height / 2, 0, 0},
            .renderData = {.text = {.stringContents =
  ...
  if (context->openLayoutElementStack.length > 1) {
    context->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData){
        ...
        .errorText =
            CLAY_STRING("There were still open layout elements when EndLayout "
                        "was called. This results from an unequal number of "
                        "calls to Clay__OpenElement and Clay__CloseElement."),
        .userData = context->errorHandler.userData});
  }
...
Clay_ScrollContainerData Clay_GetScrollContainerData(Clay_ElementId id) {
  ...
  for (int32_t i = 0; i < context->scrollContainerDatas.length; ++i) {
    ...
    if (scrollContainerData->elementId == id.id) {
      if (!scrollContainerData
               ->layoutElement) { // This can happen on the first frame before a
                                  // scroll container is declared
...
CLAY_DLL_EXPORT bool Clay_EaseOut(Clay_TransitionCallbackArguments arguments) {
  ...
  if (arguments.properties & CLAY_TRANSITION_PROPERTY_X) {
    arguments.current->boundingBox.x =
        CLAY__LERP(arguments.initial.boundingBox.x,
                   arguments.target.boundingBox.x, lerpAmount);
  }
  if (arguments.properties & CLAY_TRANSITION_PROPERTY_Y) {
    arguments.current->boundingBox.y =
        CLAY__LERP(arguments.initial.boundingBox.y,
                   arguments.target.boundingBox.y, lerpAmount);
  }
  if (arguments.properties & CLAY_TRANSITION_PROPERTY_WIDTH) {
    arguments.current->boundingBox.width =
        CLAY__LERP(arguments.initial.boundingBox.width,
                   arguments.target.boundingBox.width, lerpAmount);
  }
  if (arguments.properties & CLAY_TRANSITION_PROPERTY_HEIGHT) {
    arguments.current->boundingBox.height =
        CLAY__LERP(arguments.initial.boundingBox.height,
                   arguments.target.boundingBox.height, lerpAmount);
  }
  if (arguments.properties & CLAY_TRANSITION_PROPERTY_BACKGROUND_COLOR) {
    arguments.current->backgroundColor = CLAY__INIT(Clay_Color){
        .r = CLAY__LERP(arguments.initial.backgroundColor.r,
                        arguments.target.backgroundColor.r, lerpAmount),
        .g = CLAY__LERP(arguments.initial.backgroundColor.g,
                        arguments.target.backgroundColor.g, lerpAmount),
        .b = CLAY__LERP(arguments.initial.backgroundColor.b,
                        arguments.target.backgroundColor.b, lerpAmount),
        .a = CLAY__LERP(arguments.initial.backgroundColor.a,
                        arguments.target.backgroundColor.a, lerpAmount),
    };
  }
  if (arguments.properties & CLAY_TRANSITION_PROPERTY_OVERLAY_COLOR) {
    arguments.current->overlayColor = CLAY__INIT(Clay_Color){
        .r = CLAY__LERP(arguments.initial.overlayColor.r,
                        arguments.target.overlayColor.r, lerpAmount),
        .g = CLAY__LERP(arguments.initial.overlayColor.g,
                        arguments.target.overlayColor.g, lerpAmount),
        .b = CLAY__LERP(arguments.initial.overlayColor.b,
                        arguments.target.overlayColor.b, lerpAmount),
        .a = CLAY__LERP(arguments.initial.overlayColor.a,
                        arguments.target.overlayColor.a, lerpAmount),
    };
  }
  if (arguments.properties & CLAY_TRANSITION_PROPERTY_BORDER_COLOR) {
    arguments.current->borderColor = CLAY__INIT(Clay_Color){
        .r = CLAY__LERP(arguments.initial.borderColor.r,
                        arguments.target.borderColor.r, lerpAmount),
        .g = CLAY__LERP(arguments.initial.borderColor.g,
                        arguments.target.borderColor.g, lerpAmount),
        .b = CLAY__LERP(arguments.initial.borderColor.b,
                        arguments.target.borderColor.b, lerpAmount),
        .a = CLAY__LERP(arguments.initial.borderColor.a,
                        arguments.target.borderColor.a, lerpAmount),
    };
  }
  if (arguments.properties & CLAY_TRANSITION_PROPERTY_BORDER_WIDTH) {
    arguments.current->borderWidth = CLAY__INIT(Clay_BorderWidth){
        .left =
            (uint16_t)CLAY__LERP(arguments.initial.borderWidth.left,
                                 arguments.target.borderWidth.left, lerpAmount),
        .right = (uint16_t)CLAY__LERP(arguments.initial.borderWidth.right,
                                      arguments.target.borderWidth.right,
                                      lerpAmount),
        .top =
            (uint16_t)CLAY__LERP(arguments.initial.borderWidth.top,
                                 arguments.target.borderWidth.top, lerpAmount),
        .bottom = (uint16_t)CLAY__LERP(arguments.initial.borderWidth.bottom,
                                       arguments.target.borderWidth.bottom,
                                       lerpAmount),
        .betweenChildren = (uint16_t)CLAY__LERP(
            arguments.initial.borderWidth.betweenChildren,
            arguments.target.borderWidth.betweenChildren, lerpAmount),
    };
  }
  return ratio >= 1;
}
