...
int main(void) {
  ...
  Column("Root",
    ...
    Row("Header", .layout = {.childGap = 4}) {
      Text("Hello", .textColor = Color(255, 255, 255, 255), .fontSize = 14);
      Text("World", .textColor = Color(180, 180, 180, 255), .fontSize = 14);
    }
    /* Anonymous footer container — empty ID. */
    Row("", .layout = {.childGap = 0}) {
      Text("(footer)", .textColor = Color(120, 120, 120, 255), .fontSize = 10);
    }
    /* Box used as a decoration container — single child. */
    Box("Badge", .layout = {.sizing = {Fixed(20), Fixed(20)}},
    ...
    Button("Save", .layout = {.padding = PadAll(4)},
           .backgroundColor = Color(40, 100, 200, 255),
           .cornerRadius = RadiusAll(4)) {
      Text("Save", .textColor = Color(255, 255, 255, 255), .fontSize = 12);
    }
  }
