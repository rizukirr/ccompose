...
DefineTransition(slide_fade, 0.4f,
    .enter = { .slideX = -200, .fade = true },
...
DefineTransition(linear_slide, 0.3f,
    .handler = CC_Linear,
    .enter = { .slideY = -40 },
);

/* ---- DefineTransition: exit-only ---- */
DefineTransition(exit_only, 0.3f,
...
static void test_enter_callback(void) {
    ...
    CC_TransitionData result = cfg.enter.setInitialState(
        target, cfg.properties);
...
static void test_exit_callback(void) {
    ...
    CC_TransitionData result = cfg.exit.setFinalState(
        initial, cfg.properties);
