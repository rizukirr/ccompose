...
static void paint_canvas(CC_BoundingBox bb, void *user) {
  ...
  for (int i = 0; i < s->dot_count; i++) {
    ...
    DrawCircleV((Vector2){cx, cy}, r,
                (Color){170, 215, 220, alpha});
  }
}

/* ------- shared frame state --------------------------------------------- */

typedef enum { PAGE_HOME, PAGE_LAYOUT, PAGE_INTERACT, PAGE_SCROLL, PAGE_TRANS } Page;
...
DefineTransition(notif_anim, 0.32f,
    .enter = { .slideY = -24, .fade = true },
...
static const char *page_title(Page p) {
  switch (p) {
    ...
    case PAGE_LAYOUT:   return "Layout";
    case PAGE_INTERACT: return "Interaction";
    case PAGE_SCROLL:   return "Scroll";
    case PAGE_TRANS:    return "Transitions";
  }
  return "?";
}
...
static void NavEntry(const char *id, Page p) {
  ...
  if (CC_Clicked(id)) current_page = p;
  ...
  Button(id,
    ...
    Text(page_title(p),
         .textColor = active ? COLOR_TEXT : COLOR_MUTED,
         .fontSize  = 15);
  }
}
...
static void render_layout(void) {
  Column("LayoutRows",
         .layout = {.sizing = {Grow(), Fit()}, .childGap = 10}) {
    Text("Three Grow() children share the row evenly:",
         .fontSize = 13, .textColor = COLOR_MUTED);
    Row("RowGrow",
        .layout = {.sizing = {Grow(), Fit()}, .childGap = 8}) {
    ...
    Spacer(.height = Fixed(4));
    Text("Percent splits the remaining width:",
         .fontSize = 13, .textColor = COLOR_MUTED);
    Row("RowPct",
        .layout = {.sizing = {Grow(), Fit()}, .childGap = 8}) {
...
static void render_interact(void) {
  Row("ButtonRow",
    ...
    Button("BtnSecondary",
           .layout = {.padding = PadSymmetric(18, 10)},
           .backgroundColor = CC_Hovered("BtnSecondary") ? COLOR_HOVER
                                                         : COLOR_SURFACE_2,
           .cornerRadius    = RadiusAll(8),
           .border          = {.color = COLOR_BORDER, .width = BorderAll(1)}) {
    ...
    Button("BtnGhost",
           .layout = {.padding = PadSymmetric(18, 10)},
           .backgroundColor = CC_Hovered("BtnGhost") ? COLOR_HOVER
                                                    : COLOR_TRANSPARENT,
           .cornerRadius    = RadiusAll(8)) {
    ...
    if (CC_Clicked("BtnToggle")) toggle_compact_mode = !toggle_compact_mode;
    ...
    Button("BtnToggle",
      ...
      Box("ToggleDot",
          .layout = {.sizing = {Fixed(10), Fixed(10)}},
          .backgroundColor = toggle_compact_mode ? COLOR_ACCENT : COLOR_MUTED,
          .cornerRadius    = RadiusAll(999));
      Text(toggle_compact_mode ? "Compact mode: ON"
                               : "Compact mode: OFF",
...
static void render_scroll(void) {
  CC_ScrollUpdate(&carousel_scroll, "Carousel",
                  .horizontal = true, .drag = true, .wheel = true);

  Text("Drag with the mouse or scroll the wheel — clamped against content "
       "bounds:", .fontSize = 13, .textColor = COLOR_MUTED,
       .wrapMode = CC_TEXT_WRAP_WORDS);
  Spacer(.height = Fixed(12));

  static const char *titles[]  = {"Macros", "Layout", "Text", "Images",
                                  "Buttons", "Floating", "Scroll", "Transitions"};
  static const char *subtitles[]= {"Column / Row / Box",
                                   "Fit / Grow / Fixed",
                                   "Wrap, align, fonts",
                                   "Fit / Fill / Crop",
                                   "Hover & click queries",
...
static void render_trans(void) {
  ...
  Row("TransControls",
    ...
    Button("BtnNotifToggle",
           .layout = {.padding = PadSymmetric(18, 10)},
           .backgroundColor =
               toggle_notifications ? COLOR_PRESSED
             : CC_Hovered("BtnNotifToggle") ? COLOR_ACCENT
                                            : COLOR_ACCENT_DIM,
           .cornerRadius = RadiusAll(8)) {
      Text(toggle_notifications ? "Dismiss notification"
                                : "Show notification",
  ...
  Column("Root",
    ...
    DrawRow("Header", paint_canvas, &canvas_state,
      ...
      Column("HeaderText",
        ...
        Text("Declarative C UIs on top of Clay + raylib",
      ...
      Box("VersionPill",
          .layout = {.padding = PadSymmetric(12, 6)},
    ...
    Row("Body",
      ...
      Column("Content",
             ...
             .cornerRadius    = RadiusAll(12),
             .border          = {.color = COLOR_BORDER, .width = BorderAll(1)}) {
        Text(page_title(current_page),
             .fontSize = 26, .textColor = COLOR_TEXT);
        Text(page_blurb(current_page),
             .fontSize = 14, .textColor = COLOR_MUTED,
        ...
        switch (current_page) {
          ...
          case PAGE_LAYOUT:   render_layout();   break;
          case PAGE_INTERACT: render_interact(); break;
          case PAGE_SCROLL:   render_scroll();   break;
          case PAGE_TRANS:    render_trans();    break;
        }
      }
    }
  }
...
int main(void) {
  ...
  int global_font_id = CC_LoadGlobalFont(
      "examples/resources/Inter-Regular.ttf", 28);
  ...
  if (avatar_texture.id != 0) avatar_tex = &avatar_texture;

  CC_RUN_LOOP(frame);

  if (avatar_tex) CC_UnloadImage(avatar_texture);
  CC_Shutdown();
  return 0;
}
